...
class RequestQueryManager(models.Manager):
    def get_common_patterns(self, limit=10):
        # Get all queries
        queries = self.values_list('query', flat=True)

        # Tokenize and remove stop words
        stop_words = set(stopwords.words('english'))
        tokens = [word for query in queries for word in word_tokenize(query.lower()) if word not in stop_words]
...
class RequestQuery(models.Model):
    ...
    class Meta:
        verbose_name_plural = "Request queries"

    def __str__(self):
        return f"{self.searcher.username}: {self.query[:50]}"
